    ...
    $('.ysera-menu-wapper.vertical.support-mega-menu').each(function () {
      ...
      $(this)
        ...
        .each(function () {
          ...
          if (data_responsive != '') element_caculator = $(this).closest(data_responsive)
          ...
          if (menu_offset.left > container_left || menu_left < container_offset.left) width = container_width
          if (menu_left > container_left) width = container_width - (menu_width - (menu_left - container_left)) - 30
      ...
      if (!$('#' + this_menu_clone_id).length) {
        ...
        thisClone.find('[id]').each(function () {
          ...
          thisClone
            ...
            .attr('href', '#' + ysera_menuadd_string_prefix($(this).attr('id'), 'ysera-menu-clone-'))
          thisClone
            .find('.ysera-menu-tabs .tabs-link a[href="#' + $(this).attr('id') + '"]')
            .attr('href', '#' + ysera_menuadd_string_prefix($(this).attr('id'), 'ysera-menu-clone-'))
    ...
    if ($elem.find('.menu-item-has-children').length) {
      $elem.find('.menu-item-has-children').each(function () {
        ...
        $('.ysera-menu-clone-wrap .ysera-menu-panels').append(
          '<div id="' +
            next_nav_target +
            '" class="ysera-menu-panel ysera-menu-sub-panel ysera-menu-hidden">' +
            sub_menu_html +
            '</div>'
        )
      })
  ...
  function ysera_close_mobile_menu() {
    ...
    $(document).on('click', function (event) {
      if ($('body').hasClass('rtl')) {
        if (event.offsetX < 0) $('.ysera-menu-clone-wrap').removeClass('open')
      } else {
        if (event.offsetX > $('.ysera-menu-clone-wrap').width()) $('.ysera-menu-clone-wrap').removeClass('open')
      }
    })
  }
  ...
  $(document).on('click', '.ysera-menu-next-panel', function (e) {
    ...
    if ($(target_id).length) {
      ...
      $('.ysera-menu-panels-actions-wrap').prepend(
        '<a data-prenttitle="' +
          firstItemTitle +
          '" class="ysera-menu-prev-panel" href="#' +
          thisPanel.attr('id') +
          '" data-cur-panel="' +
          target_id +
          '" data-target="#' +
  ...
  $(document).on('click', '.ysera-menu-prev-panel', function (e) {
    ...
    if (typeof new_parent_panel_id == 'undefined' || typeof new_parent_panel_id == false) {
      ...
      var item_title = $('#' + new_parent_panel_id)
        .find('.ysera-menu-next-panel[data-target="' + target_id + '"]')
        .closest('.menu-item')
        .find('.ysera-menu-item-title')
        .attr('data-title')
